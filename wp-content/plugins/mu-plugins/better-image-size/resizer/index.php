...
//					}
		...
		function get_attachment_picture($attachment_id = 0, $sizes = array(), $attr = array())
		{
			...
			krsort($sizes); // Reverse the order for min-width media queries
			...
			foreach ($sizes as $breakpoint => $data) {
				if (intval($breakpoint) && count($data) >= 2) {
					...
					$img = bis_get_attachment_image_src($maybe_alternative_attachment_id, [$data[0], $data[1]], isset($data[2]) ? $data[2] : false);
					...
					if ($img) {
						...
						$webp_exists = file_exists(str_replace(home_url('/'), ABSPATH, $webp_src));

						// Handle retina images
						if (isset($attr['retina']) && $attr['retina']) {
							$retina = bis_get_attachment_image_src($maybe_alternative_attachment_id, [$data[0] * 2, $data[1] * 2], isset($data[2]) ? $data[2] : false);

							// Decode retina src
							$retina_src = rawurldecode($retina['src']);
							$webp_retina_src = str_replace('.' . pathinfo($retina_src, PATHINFO_EXTENSION), '.webp', $retina_src);

							if ($webp_exists) {
								$webp_src = $webp_src . ' 1x, ' . $webp_retina_src . ' 2x';
							}
							$src = $src . ' 1x, ' . $retina_src . ' 2x';
						}

						$media_query = '(min-width:' . intval($breakpoint) . 'px)';
						if ($webp_exists) {
							$html .= '<source media="' . esc_attr($media_query) . '" srcset="' . esc_attr($webp_src) . '" type="image/webp">';
						} else {
							$html .= '<source media="' . esc_attr($media_query) . '" srcset="' . esc_attr($src) . '" >';
						}
					}
				}
			}
			...
			$img_url = rawurldecode($img_url);
			$extension = pathinfo($img_url, PATHINFO_EXTENSION);
			$webp_img_url = str_replace('.' . $extension, '.webp', $img_url);
			$webp_exists = file_exists(str_replace(home_url('/'), ABSPATH, $webp_img_url));
			$mime_type = get_post_mime_type($attachment_id);

			// Add the last <source> element
			if ($webp_exists) {
				$html .= '<source srcset="' . esc_attr($webp_img_url) . '" type="image/webp">';
			} else {
				$html .= '<source srcset="' . esc_attr($img_url) . '" type="' . esc_attr($mime_type) . '">';
			}
			...
			if (!empty($attr['alt'])) {
				...
				if (empty($alt_text)) {
					...
					if ($attachment) {
						...
						// Fallback to filename if attachment not found
						$alt_text = basename(get_attached_file($attachment_id));
					}
				}
			}

			// Remove 'alt' from $attr to prevent duplication
			unset($attr['alt']);

			$img_src = $webp_exists ? $webp_img_url : $img_url;

			// Build the <img> tag, excluding srcset and sizes
			$img_attributes = '';
			// Remove 'alt' from allowed attributes
			$allowed_attr_keys = ['class', 'id', 'width', 'height', 'decoding', 'loading', 'style', 'title'];
			foreach ($attr as $key => $value) {
				if (in_array($key, $allowed_attr_keys)) {
					$img_attributes .= ' ' . $key . '="' . esc_attr($value) . '"';
				}
			}
			$html .= '<img decoding="async" loading="lazy" src="' . esc_url($img_src) . '" alt="' . esc_attr($alt_text) . '"' . $img_attributes . '>';
